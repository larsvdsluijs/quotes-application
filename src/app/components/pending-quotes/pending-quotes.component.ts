                ...
                <div class="mb-4 p-3 bg-gray-50 rounded-md">
                  ...
                  <button 
                    ...
                    Bekijk stemmen
                                 ...
                                 <!-- Vote Buttons - Only show for non-admin users -->
                 <div *ngIf="!(isAdmin$ | async)" class="flex space-x-2">
                   <button 
                 ...
                 <!-- Admin message -->
                 <div *ngIf="isAdmin$ | async" class="text-center py-3 bg-yellow-50 border border-yellow-200 rounded-md">
                   <p class="text-sm text-yellow-800">
                     <svg class="inline w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                       <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
                     </svg>
                     Admins kunnen niet stemmen op quotes
                   </p>
                 </div>
              </div>
            </div>
...
export class PendingQuotesComponent implements OnInit {
  ...
  async voteOnQuote(quoteId: string, vote: 'yes' | 'no') {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('Admins cannot vote')) {
        alert('Admins kunnen niet stemmen op quotes');
      } else {
